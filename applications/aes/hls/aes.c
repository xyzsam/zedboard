...
#pragma HLS INTERFACE axis bundle=INPUT_STREAM depth=48 port=in_stream
#pragma HLS INTERFACE axis bundle=OUTPUT_STREAM depth=16 port=out_stream
  aes256_context ctx;
  uint8_t k[32];
  uint8_t buf[16];
  uint32_t temp;
...
buffer: for (i = 0; i < 16; i+=4) {
    temp = in_stream[(i+32)/4];
#pragma HLS PIPELINE II=1
    for (j = 0; j < 4; j++) {
#pragma HLS UNROLL
      buf[i+j] = (temp >> (j*8)) & 0xff;
    }
  }

  aes256_encrypt_ecb(&ctx, k, buf);

outstream: for (i = 0; i < 16; i+=4) {
#pragma HLS PIPELINE II=1
    temp = 0;
    for (j = 0; j < 4; j++) {
#pragma HLS UNROLL
