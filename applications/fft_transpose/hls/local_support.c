...
void run_benchmark_axis(void *vargs) {
  ...
  struct bench_args_t *args_in = (struct bench_args_t *)vargs;
  struct bench_args_t args_out;
  in_stream = (TYPE*) args_in;
  out_stream = (TYPE*) &args_out;
  fft_transpose(in_stream, out_stream);
  memcpy(args_in, out_stream, sizeof(struct bench_args_t));
  hash = 0;
  for (i = 0; i < 512; i++)
    hash += args_in->work_x[i];
  for (i = 0; i < 512; i++)
    hash += args_in->work_y[i];
  printf("Result hash: %2.8f\n", hash);
}
