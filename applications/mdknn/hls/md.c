 ...
 */
void mdknn(TYPE* in_stream, TYPE* out_stream) {
...
#pragma HLS PIPELINE II=1
      position_x[i] = in_stream[i];
    }
    for (i = 0; i < nAtoms; i++) {
#pragma HLS PIPELINE II=1
      position_y[i] = in_stream[i+nAtoms];
    }
    for (i = 0; i < nAtoms; i++) {
#pragma HLS PIPELINE II=1
      position_z[i] = in_stream[i+2*nAtoms];
    }
    for (i = 0; i < nAtoms*maxNeighbors; i++) {
#pragma HLS PIPELINE II=1
      NL[i] = (int32_t)in_stream[i+3*nAtoms];
    }
...
loop_j : for( j = 0; j < maxNeighbors; j++){
    ...
    for (i = 0; i < nAtoms; i++) {
#pragma HLS PIPELINE II=1
      out_stream[i] = force_x[i];
    }
    for (i = 0; i < nAtoms; i++) {
#pragma HLS PIPELINE II=1
      out_stream[i+nAtoms] = force_y[i];
    }
    for (i = 0; i < nAtoms; i++) {
#pragma HLS PIPELINE II=1
      out_stream[i+2*nAtoms] = force_z[i];
    }
}
